    ...
    expenseRouter.delete('/:id', csrfProtection, ensureAuthenticated, async (req, res, next) => {
        ...
        await pool.query('DELETE FROM expenses WHERE id = $1', [expenseId], 
            async (err, result) => {
                ...
                let expenseTotal = await totalExpenses(userId, pool);
                await pool.query('UPDATE users SET total_expenses = $1 WHERE id = $2', [expenseTotal, userId]);
                const newExpenses = await pool.query('SELECT * FROM expenses WHERE user_id = $1', [userId]); 
                res.status(204).json({message: "Successfully deleted the entry.", expenses: newExpenses.rows})
            }
        )
    })
